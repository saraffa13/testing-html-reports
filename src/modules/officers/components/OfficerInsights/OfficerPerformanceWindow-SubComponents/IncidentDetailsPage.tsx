...
import {
  getOfficerIncidentTypeDisplayName,
  getOfficerIncidentTypeIcon,
  type OfficerIncidentType,
} from "./officer-incident-tasks-types";

// Import the API service
import {
  useIncidentDetails,
  type TransformedIncidentEvidence,
  type TransformedIncidentReport,
} from "../../../service/incidentDetailsApiService";

const IncidentDetailsPage: React.FC = () => {
  const { incidentId } = useParams<{ incidentId: string }>();
  const navigate = useNavigate();

  // Use the API service (with mock fallback)
  const { data: incidentDetails, isLoading, error, isError } = useIncidentDetails(incidentId || null, !!incidentId);
  ...
  // Map incident type string to enum for icon display
  const getIncidentTypeForIcon = (type: string): OfficerIncidentType => {
  ...
  const incidentTypeEnum = getIncidentTypeForIcon(incidentDetails.type);
              ...
              <Box sx={{ width: "572px", height: "604px", gap: "16px", display: "flex", flexDirection: "column" }}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      {React.cloneElement(getOfficerIncidentTypeIcon(incidentTypeEnum), {
                  ...
                  >
                    INCIDENT : {getOfficerIncidentTypeDisplayName(incidentTypeEnum).toUpperCase()}
                  </Typography>
