...
import DefaultsCard from "./GuardDefaultsComponent";
import type { DefaultData, GuardDefault } from "./defaults-types";

interface CalendarOverlayProps {
  open: boolean;
  onClose: () => void;
  selectedDate: Date;
  defaultsData: GuardDefault[];
  title: string;
}

/**
 * Calendar overlay component that shows defaults for week/month view
 */
const CalendarOverlay: React.FC<CalendarOverlayProps> = ({ open, onClose, selectedDate, defaultsData, title }) => {
  const [selectedDateDetails, setSelectedDateDetails] = useState<DefaultData[] | null>(null);
  ...
  const hasDefaultsOnDate = (date: Date): boolean => {
    ...
    const dayDefaults = defaultsData.find((gd) => gd.date === dateStr);
    return dayDefaults ? dayDefaults.defaults.length > 0 : false;
  };

  // Get defaults for a date
  const getDefaultsForDate = (date: Date): DefaultData[] => {
    const dateStr = format(date, "yyyy-MM-dd");
    const dayDefaults = defaultsData.find((gd) => gd.date === dateStr);
    return dayDefaults ? dayDefaults.defaults : [];
  };
  ...
  const generateCalendarDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const hasDefaults = hasDefaultsOnDate(currentDate);
  ...
  const handleDateClick = (defaults: DefaultData[], date: Date) => {
    // Only proceed if the date has defaults
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content */}
        {showingDateDetails ? (
          // Show DefaultsCard for selected date
          <Box sx={{ display: "flex", justifyContent: "center" }}>
            {selectedDateDetails && <DefaultsCard defaults={selectedDateDetails} width="502px" height="auto" />}
          </Box>
        ) : (
          // Show Calendar Grid
          <>
            {isWeekView ? (
              ...
              >
                ...
                {calendarDays.map((dayData, index) => (
                  <Box
                    key={index}
                    onClick={() => dayData && dayData.hasDefaults && handleDateClick(dayData.defaults, dayData.date)}
                    sx={{
                      padding: "12px",
                      textAlign: "center",
